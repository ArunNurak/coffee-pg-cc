...
(function($) {
  ...
  $.fn.extend({
     ...
     */
    UIMultiSelectList: function (options) {
      console.log("entered UIMultiSelectList");
      var name = (options && options.name) ? options.name : $.Uuid();
      var list = this[0];
      if (list && !$(list).hasClass('select')) {
        this.addClass('select');
      ...
      $(list).find('li').forEach(function (ctx, idx) {
        //ctx.setAttribute('role', 'radio');
        if (options && options.selected === idx && false) {
          ctx.setAttribute('aria-checked', 'true');
          ctx.classList.add('selected');
          if (!$(ctx).find('input')[0]) {
            $(ctx).append('<input type="checkbox" checked="checked" name="' + name + '">');
          } else {
            $(ctx).find('input').attr('checked', 'checked');
          }
        } else {
      ...
      $(list).on('singletap', 'li', function () {
        ...
        if(item.hasAttribute('aria-checked')){
          ...
          console.log("setting checked status");
          item.setAttribute('aria-checked', true);
          $(item).addClass('selected');
          $(item).find('input').attr('checked', 'checked');
        }
